...
//                                        wearData.shortsDisable = (celldata[14].Equals("2"));
        ...
        private static void LoadAdditionalBonesForCurrent(string assetBundlePath, string assetName, string manifest)
        {
            ...
            if (ta == null)
                return;
            _self.logSave("BonesFramework: Loaded additional_bones TextAsset from " + assetBundlePath);
            ...
            foreach (string line in lines)
            {
                ...
                if (!cells[0].Equals(assetName))
                    continue;
                _self.logSave("BonesFramework: Found matching line for asset " + assetName + "\n" + line);
        ...
        {   
            foreach (DynamicBone dynamicBone in dbs)
            {
                DynamicBone_InitTransforms.Invoke(dynamicBone, new object[0]);
                DynamicBone_SetupParticles.Invoke(dynamicBone, new object[0]);
            }
        }
        ...
        private static MethodInfo DynamicBone_InitTransforms = Assembly.GetAssembly(typeof(DynamicBone)).GetType("DynamicBone").GetMethod("InitTransforms", new Type[0]);
        private static MethodInfo DynamicBone_SetupParticles = Assembly.GetAssembly(typeof(DynamicBone)).GetType("DynamicBone").GetMethod("SetupParticles", new Type[0]);
        private static HoneyPot _self; 
        #endregion
